...
\begin{minipage}[c][16cm][c]{0.78\textwidth}
  \begin{center}
      {\sc \Huge \textbf{A PSF photometry tool for NASA's Kepler,} $\bm{\mathcal{K}\mathit{2}}$\textbf{, and TESS missions}}\\[7mm]
...
\begin{center}
\begin{pcolumn}{0.32}
\pbox{0.90\textwidth}{69cm}{linewidth=2mm,framearc=0.1,linecolor=lightblue,fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white,gradmidpoint=1.0,framesep=1em}{
...
\begin{itemize}
    ...
    \item Although Point Spread Function (PSF) photometry methods are well
        ...
        and extensible to TESS, as part of the $\mathcal{K}\mathit{2}$ Guest Observer Office data analysis tool, PyKE.
\end{itemize}
...
to the data.  In the example shown here, we fit data from the crowded Lagoon Nebula cluster
...
also benefits the light curves of faint stars. For example, star \texttt{KIC 6444896}
is known to contain a small planet, Kepler-1649b~\cite{isabel}.
Using PSF photometry, we recover the planet signal at a higher SNR.
Unlike aperture photometry, PSF photometry is not sensitive to the choice
of an aperture mask.
