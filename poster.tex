...
\begin{minipage}[c][16cm][c]{0.78\textwidth}
  \begin{center}
      ...
      {\Large \textbf{Jos\'e Vin\'icius de Miranda Cardoso}$^{1, 2, 4}$, \textbf{Geert Barentsen}$^{1, 2}$,
       \textbf{Ben Montet}$^{3, \bm{\star}}$, \textbf{Ann Marie Cody}$^{1, 2}$,}\\
      {\Large
...
\begin{itemize}
    ...
    \item However, both the official and community developed pipelines~\cite{luger, vanderburg, aigrain} tend to focus on studying
          isolated stars using simple aperture photometry, which tends to perform sub-optimally in crowded fields and on faint stars. \cite{anderson, libralato1, heasley}.
    \item Although Point Spread Function (PSF) photometry methods are well
          known~\cite{anderson, libralato1, heasley}, open source tools to perform PSF photometry specifically on Kepler and
...
of an aperture mask (which is significant for faint stars).
